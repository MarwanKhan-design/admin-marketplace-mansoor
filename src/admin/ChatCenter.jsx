...
export default function ChatCenter() {
  ...
  const loadAgents = async () => {
    const { data: profiles } = await adminSupabase.from('profiles').select('id,display_name,email').eq('role','agent').order('display_name');
    const { data: rows } = await adminSupabase.from('messages').select('*').eq('channel','agent').order('created_at',{ascending:false});
    if (profiles?.length) setAgents(profiles.map((profile) => {
      const latest = (rows || []).find((row) => row.sender_id === profile.id || row.recipient_id === profile.id);
      return { id: profile.id, userId: profile.id, name: profile.display_name || profile.email, email: profile.email, unread: latest && !latest.read_at ? 1 : 0, date: latest ? new Date(latest.created_at).toLocaleString() : '—', message: latest?.body || '' };
    }));
  };
  const loadAnnouncements = async () => {
    const { data } = await adminSupabase.from('announcements').select('*').order('created_at',{ascending:false});
    if (data) setPublished(data.map((item) => ({ id:item.id, title:item.title, content:item.message, date:new Date(item.created_at).toLocaleDateString() })));
  };
  useEffect(()=>{
    loadSellerMessages(); loadAgents(); loadAnnouncements();
    const channel = adminSupabase.channel('admin-message-center').on('postgres_changes',{event:'*',schema:'public',table:'messages'},()=>{ loadSellerMessages(); loadAgents(); }).on('postgres_changes',{event:'*',schema:'public',table:'announcements'},loadAnnouncements).subscribe();
  ...
  const sendThreadReply=async()=>{
    ...
    const {data:auth}=await adminSupabase.auth.getUser();
    await adminSupabase.from('messages').insert({sender_id:auth.user.id,recipient_id:activeThread.userId,channel:activeThread.type === 'agent' ? 'agent' : 'service',body:threadReply.trim()});
    setThreadReply('');setActiveThread(null);loadSellerMessages();loadAgents();
  };

  const publishAnnouncement = async (event) => {
    event.preventDefault();
    const { data: auth } = await adminSupabase.auth.getUser();
    await adminSupabase.from('announcements').insert({ title: announcementForm.title.trim(), message: announcementForm.content.trim(), target_type: 'all', created_by: auth.user.id });
  ...
  return <section className="chat-center-page">
    ...
    {activeTab === 'Agent Chats' && <div className="agent-chats-view">
      ...
      <div className="agent-channel-list">{agents.map((agent) => <article key={agent.id}><div className="agent-chat-avatar">{agent.name[0].toUpperCase()}</div><div><strong>{agent.name}</strong><span>{agent.email}</span><small>{String(agent.id).slice(0,8).toUpperCase()}</small></div><time>{agent.date}</time>{agent.unread > 0 && <b className="channel-unread">{agent.unread}</b>}<button type="button" onClick={() => setActiveThread({ type: 'agent', userId: agent.userId || agent.id, ...agent })}>◯ Open Chat</button></article>)}</div>
    ...
    {activeTab === 'Announcements' && <div className="chat-announcements-view">
      ...
      <h3 className="published-heading">Published Announcements</h3><div className="published-list">{published.map((item) => <article key={item.id}><strong>{item.title}</strong><span>{item.content}</span><small>{item.date}</small><button type="button" onClick={async () => { await adminSupabase.from('announcements').delete().eq('id',item.id); loadAnnouncements(); }}>♲</button></article>)}</div>
    </div>}
