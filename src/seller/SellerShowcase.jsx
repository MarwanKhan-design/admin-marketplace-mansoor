...
export default function SellerShowcase({ client, sellerId, shopLocked, onBack }) {
  ...
  useEffect(() => {
    ...
    const loadCloudShowcase = async () => {
      ...
      if (products?.length) {
        const mapped = products.map((item) => ({
          ...
          onShelf: item.admin_on_shelf,
        }));
        setCatalog(mapped);
        const { data: selected } = await client
          .from("showcase_products")
          .select("product_id,on_shelf")
          .eq("seller_id", sellerId);
        if (selected?.length)
          setSelections(
            selected
              .map((item) => ({
                id: mapped.find((product) => product.dbId === item.product_id)
                  ?.id,
                onShelf: item.on_shelf,
              }))
              .filter((item) => item.id),
          );
      }
    ...
    const channel = client
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "showcase_products" },
        loadCloudShowcase,
      )
      .subscribe();
    ...
    () =>
      selections
        .map((selection) => {
          ...
          return product
            ? { ...product, sellerOnShelf: selection.onShelf }
            : null;
        })
        .filter(Boolean),
    [catalog, selections],
  ...
  const visibleProducts = selectedProducts.filter(
    (product) =>
      ...
      (shelf === "All" ||
        (shelf === "On Shelf"
          ? product.sellerOnShelf
          : !product.sellerOnShelf)),
  );
  const availableProducts = catalog.filter(
    (product) =>
      !selections.some((selection) => selection.id === product.id) &&
      (category === "All" || product.category === category),
  );
  ...
  const addProduct = async (product) => {
    ...
    if (client && sellerId)
      await client.from("showcase_products").upsert({
        seller_id: sellerId,
  ...
  const toggleShelf = async (id) => {
    ...
    saveSelections(
      selections.map((item) => (item.id === id ? { ...item, onShelf } : item)),
    );
    if (client && sellerId)
      await client
        .from("showcase_products")
        .update({ on_shelf: onShelf })
        .eq("seller_id", sellerId)
  ...
  if (adding)
    return (
      <main className="seller-showcase-page">
        <div className="seller-showcase-shell add-products-shell">
          <header>
            <button
              type="button"
              onClick={() => {
          ...
          <nav className="seller-showcase-categories">
            {categories.map((item) => (
              <button
          ...
          <p className="add-products-help">
            Select products from the admin catalog to add to your showcase
          </p>
          <section className="add-products-list">
  ...
  return (
    <main className="seller-showcase-page">
      <div className="seller-showcase-shell">
        ...
        <nav className="seller-shelf-tabs">
          {["On Shelf", "All", "Off Shelf"].map((item) => (
            <button
        ...
        <section className="seller-showcase-products">
          {visibleProducts.map((product) => (
            <article key={product.id}>
              <h2>Product No: {product.sku}</h2>
              ...
              <div className="seller-product-prices">
                <div>
                  <strong>${product.sellPrice.toFixed(2)}</strong>
                  <span>Sell Price</span>
                </div>
                <div>
                  <strong>${product.costPrice.toFixed(2)}</strong>
              ...
              <button
                className={product.sellerOnShelf ? "on" : "off"}
                type="button"
                onClick={() => toggleShelf(product.id)}
              >
                ◉ &nbsp; {product.sellerOnShelf ? "ON SHELF" : "OFF SHELF"}
              </button>
            </article>
          ))}
          {!visibleProducts.length && (
            <div className="seller-showcase-empty">No products found.</div>
          )}
        </section>
      </div>
    </main>
