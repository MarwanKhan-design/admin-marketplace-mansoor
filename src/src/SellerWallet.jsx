...
export default function SellerWallet({ onBack }) {
  const [filter, setFilter] = useState('All');
  const [dialog, setDialog] = useState(null);
  const [amount, setAmount] = useState('');
  const [walletView, setWalletView] = useState('wallet');
  const visibleTransactions = useMemo(() => filter === 'All' ? transactions : transactions.filter((item) => item.type === filter), [filter]);
  ...
  return <main className="seller-wallet-page"><div className="seller-wallet-shell">
    ...
    <section className="wallet-balances"><div><strong>$4,752.92</strong><span>My Assets</span></div><div><strong>$600.00</strong><span>Frozen</span></div><div><strong>$0.00</strong><span>Settled</span></div><div><strong>$0.00</strong><span>Unsettled</span></div></section>
